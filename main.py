...
async def btn_commands(message: Message):
    ...
    text = (
        ...
        "• `.purge <кол-во>` — Быстро зачистить N последних сообщений от вашего имени."
    )
...
async def on_business_connection(connection: BusinessConnection):
    ...
    if is_enabled:
        ...
        text = (
            ...
            f"🆔 Connection ID: `{connection_id}`\n"
            "🔴 Бот больше не имеет доступа к вашим чатам. Для повторного включения загляните в настройки Telegram."
        )
...
async def on_business_message(message: Message):
    ...
    # =================================================================
    # 3. СОХРАНЕНИЕ В БАЗУ ДАННЫХ С УЧЕТОМ НАСТРОЕК
    # =================================================================
    if owner_id:
        settings = await get_user_settings(owner_id)

        # Проверка настройки "Только сообщения собеседника"
        ...
        # Проверка настройки сохранения медиафайлов
